...
/*function replacePlaceholdersInPlist(plistPath, placeHolderValues) {
    ...
    for (var i = 0; i < placeHolderValues.length; i++) {
        ...
        console.log(">>>placeHolderValue.placeHolder: " + placeHolderValue.placeHolder);
        console.log(">>>placeHolderValue.value: " + placeHolderValue.value);
...
function replacePlaceholdersInPlist(plistPath, placeHolder,value) {
    var plistContents = fs.readFileSync(plistPath, 'utf8');
        plistContents = plistContents.replace(placeHolder, value);
        plistContents = removeDuplicateSubsequentLines(plistContents);
    fs.writeFileSync(plistPath, plistContents);
}
...
module.exports = function (context) {
  ...
  fs.readdir(iosFolder, function (err, data) {
    ...
    var run = function () {
      ...
      var value = "\t<key>com.apple.CommCenter.fine-grained</key>\n\t<array>\n\t\t<string>public-cellular-plan</string>\n\t</array>\n    </dict>";
      ...
      ['Debug', 'Release'].forEach(config => {
        ...
        replacePlaceholdersInPlist(entitlementsPath, placeHolder, value);
      });
      log('Successfully added the eSIM capability to the app entitlements files!', 'success');
